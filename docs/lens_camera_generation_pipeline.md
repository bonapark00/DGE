...
4. **SAGE 점수** (`_lens_compute_sage_score`):
  ...
  - **총점**:  
  \text{S}*{\text{total}} = \text{focus} - \lambda*{\text{leak}} \cdot \text{leakage} - \lambda_{\text{ent}} \cdot \text{entropy} - \text{sizepenalty}.
5. 점수가 최대인 d를 d^*로 선택. 모두 -\infty면 `distance_multipliers` 중앙값을 fallback으로 사용.
...
1. **Fibonacci 구면 샘플** (`_lens_fibonacci_sphere_samples(n_candidates)`):
  golden angle \pi(3-\sqrt{5}) 기반으로 구면 위에 n개 방향 생성.
2. **필터** (선택):
...
3. **정면/측면 점수 S_{\text{can}}**:
  - 뷰 방향 \mathbf{v}_{\text{view}} = \text{normalize}(\text{center} - \text{camcenter}).
  - \cos_{\text{front}} = |\mathbf{v}*{\text{view}} \cdot \mathbf{v}*{\text{front}}|,  
...
**Key-view 선택기로 사용 (Multiview edit)**  
- Step 5의 while 루프에서 **맨 처음 선택되는 K개**가 에너지 1위 + (기존 선택과 각도 거리 × 에너지) 순서이다.  
- **방위각 정렬 전** 이 순서를 그대로 Key-view로 쓰면 된다.  
- **기존 카메라 풀**에서 Key를 고를 때: `select_key_views_by_lens_fps(gaussians, cameras, n_key, ...)` 로 동일 알고리즘 적용 (에너지 스코어 → FPS, 처음 K개 반환).  
- **LENS로 카메라 생성** 시: `edit_view_indices` = 방위각 정렬된 리스트에서 **앞에서부터** max_edit_view_num개 (인덱스 0..K-1).
...
- **Key-view 선택 (기존 카메라 풀)**: `select_key_views_by_lens_fps` (같은 파일).  
- **DGE multiview key 선택 전략**: `multiview_edit_key_selection_strategy: "lens_fps"` (DGE 시스템 설정).
