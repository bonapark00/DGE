...
for max_view_num in "${MAX_VIEW_NUMS[@]}"; do
  ...
  if [[ ! -d "$exp_folder/save" ]]; then
    echo "  WARNING: No save directory found. Removing folder: $exp_folder"
    rm -rf "$exp_folder"
    SKIPPED+=("$exp_name (no save dir, removed)")
    echo ""
    continue
  fi
  ...
  # Extract latency from latency/summary.txt if it exists
  latency_summary="$exp_folder/latency/summary.txt"
  latency_time=""
  if [[ -f "$latency_summary" ]]; then
    # Extract time from first line like "Latency Summary (seconds) - Total Time: 27.936s"
    latency_line=$(head -n 1 "$latency_summary")
    # Use grep to extract the time value
